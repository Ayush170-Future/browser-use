...
async def execute_task(task: str, use_global_context: bool, feature: str | None = None, planner_req: bool = False):
	...
	if use_global_context and global_context:
		...
		context = await browser.new_context(
			config=BrowserContextConfig(trace_path='./tmp/traces/', browser_window_size={'width': 1920, 'height': 1080})
		)
		if use_global_context:
			global_context = context

	agent = Agent(task=task, llm=llm, use_vision=False, browser_context=context, planner_llm=llm, max_failures=5, browser=browser)

	if not planner_req:
		agent = Agent(task=task, llm=llm, use_vision=False, browser_context=context, max_failures=5, browser=browser)
...
async def get_current_page():
	global agent
	if agent is None:
		return {'success': True, 'msg': 'No agent found', 'current_page_html': '', 'current_markdown': '', 'url': '', 'tabs': []}
	else:
